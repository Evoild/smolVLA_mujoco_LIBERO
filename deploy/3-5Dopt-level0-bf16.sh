...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-5/D-opt-level0-bf16"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --engine-output PATH       default: OUTPUT_ROOT/smolvla_debug_core_native_mixed_opt_level0.plan" \
...
mkdir -p "$OUTPUT_ROOT"
ENGINE_OUTPUT="${ENGINE_OUTPUT:-$OUTPUT_ROOT/smolvla_debug_core_native_mixed_opt_level0.plan}"
...
if [[ "$BUILD_ENGINE" == "true" ]]; then
  ...
  trtexec \
    ...
    --timingCacheFile="$OUTPUT_ROOT/smolvla_debug_bf16_opt_level0.cache"
...
printf '%s\n' "step 3-5D optimization-level-0 BF16 outputs:" \
  "  BF16 source ONNX: $BF16_ONNX" \
  "  ORT-compatible BF16 shadow ONNX: $ORT_BF16_ONNX" \
  "  TensorRT optimization-level-0 engine: $ENGINE_OUTPUT" \
