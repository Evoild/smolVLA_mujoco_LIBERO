...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/5/O-smoothquant-fake-dequant"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --smooth-alpha A               default: $SMOOTH_ALPHA" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --smooth-alpha) require_value "$@"; SMOOTH_ALPHA="$2"; shift 2 ;;
...
SMOOTH_SCALES_JSON="$OUTPUT_ROOT/smoothquant_alpha_${SMOOTH_ALPHA}_activation_scales.json"
...
if [[ "$RUN_CALIBRATION" == "true" ]]; then
  ...
  [[ -f "$CHANNEL_SCALES_JSON" ]] || die "$CHANNEL_SCALES_JSON not found; enable --run-calibration true"
fi
...
printf '%s\n' "step 5O SmoothQuant fake-dequant outputs:" \
  "  output root: $OUTPUT_ROOT" \
  "  source per-channel scales: $CHANNEL_SCALES_JSON" \
  "  SmoothQuant scales: $SMOOTH_SCALES_JSON" \
