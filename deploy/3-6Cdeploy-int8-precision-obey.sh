...
ENGINE_PATH="$REPO_ROOT/runs/deploy/3-5/C-int8-precision-obey/smolvla_debug_core_vlm_w8a8_precision_obey.plan"
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-6/C-int8-precision-obey"
...
[[ -f "$ENGINE_PATH" ]] || die "$ENGINE_PATH not found; run deploy/3-5Cint8-precision-obey-engine.sh first"

export LEROBOT_SRC="${LEROBOT_SRC:-$REPO_ROOT/lerobot/src}"
export PYTHONPATH="$LEROBOT_SRC${PYTHONPATH:+:$PYTHONPATH}"
mkdir -p "$OUTPUT_ROOT/trt-int8"
...
if [[ "$RUN_PROFILE" == "true" ]]; then
  "$PYTHON" "$SCRIPT_DIR/trt_sample_actions_core_deploy.py" profile \
    ...
    --device "$DEVICE" \
    --output-dir "$OUTPUT_ROOT/trt-int8/profile" \
    --warmup "$PROFILE_WARMUP" \
    --iters "$PROFILE_ITERS" \
    --task "libero_goal trt-int8 precision-obey sample_actions inference profile"
fi
...
if [[ "$RUN_EVAL" == "true" ]]; then
  "$PYTHON" "$SCRIPT_DIR/trt_sample_actions_core_deploy.py" eval \
    ...
    --policy-path "$POLICY_PATH" \
    --device "$DEVICE" \
    --output-dir "$OUTPUT_ROOT/trt-int8/eval" \
    --tasks "$TASK_SUITE" \
    --seed "$SEED" \
    --episodes "$EPISODES" \
...
printf '%s\n' "step 3-6C INT8 precision-obey deployment outputs:" \
  "  engine: $ENGINE_PATH" \
  "  profile summary: $OUTPUT_ROOT/trt-int8/profile/profile_summary.json" \
  "  eval info: $OUTPUT_ROOT/trt-int8/eval/eval_info.json" \
  "  profile metrics include latency, throughput/fps, and peak GPU memory" \
  "  eval metrics include success rate and eval peak GPU memory"
