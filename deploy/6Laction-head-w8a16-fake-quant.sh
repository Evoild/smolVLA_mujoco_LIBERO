...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/6/L-alpha085-text-a8w8-lm-expert-w8a16-fake-quant"
...
# Primary baseline:
#   SmoothQuant alpha=0.85 text_model A8W8 for q/k/v/o + FFN gate/up/down Linear.
# Extra replacement:
#   lm_expert/action-head 32 transformer layers are directly replaced with W8A16.
TEXT_A8W8_REGEX="^model\\.vlm_with_expert\\.vlm\\.model\\.text_model\\.layers\\.[0-9]+\\.(self_attn\\.(q_proj|k_proj|v_proj|o_proj)|mlp\\.(gate_proj|up_proj|down_proj))$"
ACTION_HEAD_W8A16_REGEX="^model\\.vlm_with_expert\\.lm_expert\\.layers\\.[0-9]+\\.(self_attn\\.(q_proj|k_proj|v_proj|o_proj)|mlp\\.(gate_proj|up_proj|down_proj))$"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --text-a8w8-regex RE       default: $TEXT_A8W8_REGEX" \
    "  --action-head-w8a16-regex RE default: $ACTION_HEAD_W8A16_REGEX" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --text-a8w8-regex) require_value "$@"; TEXT_A8W8_REGEX="$2"; shift 2 ;;
    --action-head-w8a16-regex) require_value "$@"; ACTION_HEAD_W8A16_REGEX="$2"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
[[ -f "$SCALES_JSON" ]] || die "missing SmoothQuant alpha=0.85 scales: $SCALES_JSON"
...
"$PYTHON" "$SCRIPT_DIR/diagnose_3_4_numeric_baseline.py" compare \
  ...
  --quantize-module-regex "$TEXT_A8W8_REGEX" \
  --fake-quant-kind w8a8 \
  --fake-quant-activation-scale-mode calibrated \
  --fake-quant-activation-scales-json "$SCALES_JSON" \
  --extra-w8a16-module-regex "$ACTION_HEAD_W8A16_REGEX" \
  --output-dir "$OUTPUT_ROOT/fake_dequant_numeric"

printf '%s\n' "step 6L SmoothQuant alpha=0.85 text_model A8W8 + lm_expert W8A16 fake-quant outputs:" \
  ...
  "  text_model A8W8 regex: $TEXT_A8W8_REGEX" \
  "  lm_expert W8A16 regex: $ACTION_HEAD_W8A16_REGEX"
