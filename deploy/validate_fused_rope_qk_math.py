...
"""Validate fused RoPE/layout/QK score math against a PyTorch reference."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-json", default="runs/deploy/6/D-fused-rope-qk/fused_rope_qk_math_check.json")
...
def qk_ref(q_raw: torch.Tensor, k_raw: torch.Tensor, positions: torch.Tensor, q_heads: int, kv_heads: int, head_dim: int):
...
def qk_fused_ref(q_raw: torch.Tensor, k_raw: torch.Tensor, positions: torch.Tensor, q_heads: int, kv_heads: int, head_dim: int):
    bsz, seq_len, _ = q_raw.shape
    groups = q_heads // kv_heads
    q = apply_rope_ref(q_raw.view(bsz, seq_len, q_heads, head_dim), positions)
    k = apply_rope_ref(k_raw.view(bsz, seq_len, kv_heads, head_dim), positions)
    k = k[:, :, :, None, :].expand(bsz, seq_len, kv_heads, groups, head_dim)
    k = k.reshape(bsz, seq_len, q_heads, head_dim)
...
def main() -> None:
    ...
    ref = qk_ref(q_raw, k_raw, positions, args.q_heads, args.kv_heads, args.head_dim)
    fused = qk_fused_ref(q_raw, k_raw, positions, args.q_heads, args.kv_heads, args.head_dim)
    causal_mask = torch.ones(args.batch, 1, args.seq_len, args.seq_len, device=device, dtype=torch.bool).tril()
    scale = args.head_dim**-0.5
    ref_probs = torch.softmax((ref * scale).masked_fill(~causal_mask, torch.finfo(torch.float32).min), dim=-1)
    fused_probs = torch.softmax((fused * scale).masked_fill(~causal_mask, torch.finfo(torch.float32).min), dim=-1)
    report = {
        "device": str(device),
        "dtype": args.dtype,
        "batch": args.batch,
        ...
        "score_output_shape": list(fused.shape),
        "score_metrics": metrics(ref, fused),
        "softmax_metrics": metrics(ref_probs, fused_probs),
    }
