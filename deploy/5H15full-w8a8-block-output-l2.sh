...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/5/H-bf16-int8-w8a8/full_w8a8_block_outputs"
...
# W8A8:
#   q/k/v/o + gate/up/down: all 32 Text VLM layers
...
SCALES_JSON="$OUTPUT_ROOT/activation_scales_${CALIBRATION_SAMPLES}_p${CALIBRATION_PERCENTILE}.json"
...
"$PYTHON" "$SCRIPT_DIR/compare_5h_transformer_block_outputs.py" \
  ...
  --output-dir "$OUTPUT_ROOT/block_outputs" \
  --device "$DEVICE" \
  --seed "$SEED" \
  --task "$TASK_SUITE" \
  --batch-size "$BATCH_SIZE" \
...
printf '%s\n' "step 5H.15 full W8A8 transformer block output L2 outputs:" \
  "  output root: $OUTPUT_ROOT" \
  "  activation scales: $SCALES_JSON" \
  "  block summary: $OUTPUT_ROOT/block_outputs/text_vlm_block_output_l2_summary.json" \
  "  relative L2 curve: $OUTPUT_ROOT/block_outputs/text_vlm_block_output_relative_l2.png" \
  "  absolute L2 curve: $OUTPUT_ROOT/block_outputs/text_vlm_block_output_absolute_l2.png" \
  "  BF16 norm curve: $OUTPUT_ROOT/block_outputs/text_vlm_block_output_ref_norm.png" \
