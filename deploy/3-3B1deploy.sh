...
ENGINE_PATH="$REPO_ROOT/runs/deploy/3-3B1/smolvla_sample_actions_core_w8a8_int8.plan"
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-3B1/deploy"
...
if [[ "$BACKEND" != "pytorch" && ! -f "$ENGINE_PATH" ]]; then
  die "$ENGINE_PATH not found; run deploy/3-3B1export.sh first"
fi
...
run_backend() {
  ...
  if [[ "$RUN_PROFILE" == "true" ]]; then
    "$PYTHON" "$SCRIPT_DIR/trt_sample_actions_core_deploy.py" profile \
      ...
      --iters "$PROFILE_ITERS" \
      --task "libero_goal $backend sample_actions inference profile"
  fi
