...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/5/H-bf16-int8-w8a8/sample_size_sweep"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --sample-sizes LIST            default: ${SAMPLE_SIZES[*]}" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --sample-sizes) require_value "$@"; read -r -a SAMPLE_SIZES <<< "$2"; shift 2 ;;
...
for sample_size in "${SAMPLE_SIZES[@]}"; do
  run_dir="$OUTPUT_ROOT/samples_${sample_size}"
  scales_json="$run_dir/activation_scales_${sample_size}.json"
  mkdir -p "$run_dir"
...
"$PYTHON" "$SCRIPT_DIR/summarize_5h_sample_size_sweep.py" \
  --output-root "$OUTPUT_ROOT" \
  --sample-sizes "${SAMPLE_SIZES[@]}"

printf '%s\n' "step 5H calibration sample-size sweep outputs:" \
  "  output root: $OUTPUT_ROOT" \
  "  summary json: $OUTPUT_ROOT/calibration_sample_size_sweep_summary.json" \
  "  summary csv: $OUTPUT_ROOT/calibration_sample_size_sweep_summary.csv" \
