...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-5/C-no-tf32-bf16"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --engine-output PATH       default: OUTPUT_ROOT/smolvla_debug_core_native_mixed_no_tf32.plan" \
...
mkdir -p "$OUTPUT_ROOT"
ENGINE_OUTPUT="${ENGINE_OUTPUT:-$OUTPUT_ROOT/smolvla_debug_core_native_mixed_no_tf32.plan}"
...
if [[ "$BUILD_ENGINE" == "true" ]]; then
  ...
  trtexec \
    ...
    --timingCacheFile="$OUTPUT_ROOT/smolvla_debug_bf16_no_tf32.cache"
