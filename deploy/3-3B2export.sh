...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-3B2"
BF16_ONNX=""
INT8_ONNX=""
ENGINE_OUTPUT=""
DEVICE=cuda
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --int8-onnx PATH           default: OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_dynamic_qdq.onnx" \
    "  --engine-output PATH       default: OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_dynamic.plan" \
...
INT8_ONNX="${INT8_ONNX:-$OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_dynamic_qdq.onnx}"
ENGINE_OUTPUT="${ENGINE_OUTPUT:-$OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_dynamic.plan}"
...
if [[ "$BUILD_ENGINE" == "true" ]]; then
  ...
  trtexec \
    ...
    --timingCacheFile="$OUTPUT_ROOT/smolvla_w8a8_dynamic.cache"
  if [[ ! -s "$ENGINE_OUTPUT" ]]; then
    die "TensorRT reported build completion but produced an empty/missing engine: $ENGINE_OUTPUT"
  fi
  engine_size="$(stat -c%s "$ENGINE_OUTPUT")"
  if (( engine_size < 1048576 )); then
    die "TensorRT produced an invalid tiny engine (${engine_size} bytes): $ENGINE_OUTPUT"
  fi
fi

printf '%s\n' "step 3-3B2 export outputs:" \
  "  bf16 source ONNX: $BF16_ONNX" \
  "  dynamic INT8 Q/DQ ONNX: $INT8_ONNX" \
