...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/6/M-alpha085-text-a8w8-lm-expert-ffn-w8a16-fake-quant"
...
# Extra replacement:
#   only lm_expert/action-head FFN gate/up/down Linear are directly replaced with W8A16.
...
LM_EXPERT_FFN_W8A16_REGEX="^model\\.vlm_with_expert\\.lm_expert\\.layers\\.[0-9]+\\.mlp\\.(gate_proj|up_proj|down_proj)$"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --lm-expert-ffn-w8a16-regex RE default: $LM_EXPERT_FFN_W8A16_REGEX" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --lm-expert-ffn-w8a16-regex) require_value "$@"; LM_EXPERT_FFN_W8A16_REGEX="$2"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
"$PYTHON" "$SCRIPT_DIR/diagnose_3_4_numeric_baseline.py" compare \
  ...
  --extra-w8a16-module-regex "$LM_EXPERT_FFN_W8A16_REGEX" \
  --output-dir "$OUTPUT_ROOT/fake_dequant_numeric"

printf '%s\n' "step 6M SmoothQuant alpha=0.85 text_model A8W8 + lm_expert FFN W8A16 fake-quant outputs:" \
  ...
  "  lm_expert FFN W8A16 regex: $LM_EXPERT_FFN_W8A16_REGEX"
