...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-3B1"
BF16_ONNX=""
INT8_ONNX=""
ENGINE_OUTPUT=""
DEVICE=cuda
...
TOKEN_LENGTH=48
OPSET=18
ACTIVATION_SCALE="0.007874015748031496"
CHECK=true
BUILD_ENGINE=true
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --int8-onnx PATH           default: OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_int8_qdq.onnx" \
    "  --engine-output PATH       default: OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_int8.plan" \
    "  --device DEVICE            default: $DEVICE" \
    "  --batch-size N             default: $BATCH_SIZE" \
    "  --token-length N           default: $TOKEN_LENGTH" \
    "  --opset N                  default: $OPSET" \
    "  --activation-scale FLOAT   static activation Q/DQ scale, default: $ACTIVATION_SCALE" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --activation-scale) require_value "$@"; ACTIVATION_SCALE="$2"; shift 2 ;;
...
INT8_ONNX="${INT8_ONNX:-$OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_int8_qdq.onnx}"
ENGINE_OUTPUT="${ENGINE_OUTPUT:-$OUTPUT_ROOT/smolvla_sample_actions_core_w8a8_int8.plan}"
...
qdq_args=(
  --input "$BF16_ONNX"
  --output "$INT8_ONNX"
  --activation-scale "$ACTIVATION_SCALE"
)
if [[ "$CHECK" == "true" ]]; then
  qdq_args+=(--check)
fi
...
if [[ "$BUILD_ENGINE" == "true" ]]; then
  ...
  trtexec \
    ...
    --timingCacheFile="$OUTPUT_ROOT/smolvla_w8a8_int8.cache"
fi

printf '%s\n' "step 3-3B1 export outputs:" \
