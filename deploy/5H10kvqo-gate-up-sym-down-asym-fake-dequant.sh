...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/5/H-bf16-int8-w8a8/kvqo_gate_up_sym_down_asym"
...
# W8A8:
#   q/k/v/o: symmetric activation A8 + symmetric per-output-channel W8
#   gate/up: symmetric activation A8 + symmetric per-output-channel W8
#   down: asymmetric activation A8 + symmetric per-output-channel W8
...
ASYMMETRIC_MODULE_REGEX="^model\\.vlm_with_expert\\.vlm\\.model\\.text_model\\.layers\\.[0-9]+\\.mlp\\.down_proj$"
...
SCALES_JSON="$OUTPUT_ROOT/activation_scales_${CALIBRATION_SAMPLES}_p${CALIBRATION_PERCENTILE}_down_asym.json"
...
if [[ "$RUN_CALIBRATION" == "true" ]]; then
  "$PYTHON" "$SCRIPT_DIR/calibrate_smolvla_activation_scales.py" \
    ...
    --asymmetric-module-regex "$ASYMMETRIC_MODULE_REGEX"
else
...
printf '%s\n' "step 5H.10 KVQO+gate/up symmetric, down asymmetric fake-dequant outputs:" \
