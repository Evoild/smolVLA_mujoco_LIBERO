...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-3B4"
SCALES_JSON=""
BF16_ONNX=""
INT8_ONNX=""
ENGINE_OUTPUT=""
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --int8-onnx PATH           default: OUTPUT_ROOT/smolvla_sample_actions_core_vlm_w8a8_qdq.onnx" \
    "  --engine-output PATH       default: OUTPUT_ROOT/smolvla_sample_actions_core_vlm_w8a8.plan" \
    ...
    "  --run-onnx-export true|false default: $RUN_ONNX_EXPORT" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --run-onnx-export) require_value "$@"; RUN_ONNX_EXPORT="$2"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
case "$RUN_ONNX_EXPORT" in true|false) ;; *) die "--run-onnx-export must be true or false" ;; esac
...
INT8_ONNX="${INT8_ONNX:-$OUTPUT_ROOT/smolvla_sample_actions_core_vlm_w8a8_qdq.onnx}"
ENGINE_OUTPUT="${ENGINE_OUTPUT:-$OUTPUT_ROOT/smolvla_sample_actions_core_vlm_w8a8.plan}"
...
if [[ "$RUN_ONNX_EXPORT" == "true" ]]; then
  if [[ "$CHECK" == "true" ]]; then
    export_args+=(--check)
  fi
  "$PYTHON" "$SCRIPT_DIR/export_smolvla_sample_actions_onnx.py" "${export_args[@]}"
else
  [[ -f "$BF16_ONNX" ]] || die "$BF16_ONNX not found; enable --run-onnx-export true or pass --bf16-onnx"
fi
