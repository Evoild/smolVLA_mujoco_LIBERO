...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/5/O-smoothquant-alpha085-full-w8a8-deploy-cudagraph"
...
LM_EXPERT_MODULE_REGEX="^model\\.vlm_with_expert\\.lm_expert\\.layers\\.[0-9]+\\.(self_attn\\.(q_proj|k_proj|v_proj|o_proj)|mlp\\.(gate_proj|up_proj|down_proj))$"
W8A8_MODULE_REGEX="^(model\\.vlm_with_expert\\.vlm\\.model\\.text_model\\.layers|model\\.vlm_with_expert\\.lm_expert\\.layers)\\.[0-9]+\\.(self_attn\\.(q_proj|k_proj|v_proj|o_proj)|mlp\\.(gate_proj|up_proj|down_proj))$"
W8A8_NODE_REGEX="^/(debug_core/)?(q_proj|k_proj|v_proj|o_proj|mlp/(gate_proj|up_proj|down_proj))(_([1-9]|[12][0-9]|3[01]))?/MatMul$"
EXPECTED_W8A8_NODES=224
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --scales-json PATH            default: output-root/smoothquant_alpha_0.85_lm_expert_activation_scales.json" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --scales-json) require_value "$@"; SCALES_JSON_OVERRIDE="$2"; shift 2 ;;
...
for value in "$BUILD_ENGINE" "$RUN_PROFILE" "$RUN_EVAL" "$RUN_NUMERIC_COMPARE" "$USE_CUDA_GRAPH" "$RUN_CALIBRATION"; do
...
QDQ_ONNX="$OUTPUT_ROOT/smolvla_action_only_smoothquant_alpha085_full_w8a8_qdq.onnx"
ENGINE_OUTPUT="$OUTPUT_ROOT/smolvla_action_only_smoothquant_alpha085_full_w8a8_precision_${TRT_PRECISION_CONSTRAINTS}.plan"
CACHE_FILE="$OUTPUT_ROOT/smolvla_action_only_smoothquant_alpha085_full_w8a8_precision_${TRT_PRECISION_CONSTRAINTS}.cache"
SUMMARY_JSON="$OUTPUT_ROOT/deploy_eval/deployment_summary.json"
SUMMARY_MD="$OUTPUT_ROOT/deploy_eval/deployment_summary.md"
CHANNEL_SCALES_JSON="$OUTPUT_ROOT/activation_channel_scales_${CALIBRATION_SAMPLES}_p${CALIBRATION_PERCENTILE}_lm_expert.json"
LM_EXPERT_SCALES_JSON="$OUTPUT_ROOT/smoothquant_alpha_0.85_lm_expert_activation_scales.json"
SCALES_JSON="${SCALES_JSON_OVERRIDE:-$OUTPUT_ROOT/smoothquant_alpha_0.85_text_and_lm_expert_activation_scales.json}"
...
if [[ "$RUN_CALIBRATION" == "true" || ! -f "$LM_EXPERT_SCALES_JSON" ]]; then
  "$PYTHON" "$SCRIPT_DIR/calibrate_smolvla_activation_channel_scales.py" \
    ...
    --include-module-regex "$LM_EXPERT_MODULE_REGEX"

  "$PYTHON" "$SCRIPT_DIR/build_smoothquant_scales.py" \
    --policy-path "$POLICY_PATH" \
    --activation-channel-scales-json "$CHANNEL_SCALES_JSON" \
    --output "$LM_EXPERT_SCALES_JSON" \
    --device "$DEVICE" \
    --alpha 0.85 \
    --include-module-regex "$LM_EXPERT_MODULE_REGEX"
fi

if [[ -z "${SCALES_JSON_OVERRIDE:-}" ]]; then
  "$PYTHON" - "$TEXT_SCALES_JSON" "$LM_EXPERT_SCALES_JSON" "$SCALES_JSON" <<'PY'
import json
import sys
from pathlib import Path

text_path, expert_path, output_path = map(Path, sys.argv[1:])
text = json.loads(text_path.read_text())
expert = json.loads(expert_path.read_text())
merged = {
    **text,
    "source_text_model_smoothquant_scales_json": str(text_path),
    "source_lm_expert_smoothquant_scales_json": str(expert_path),
    "include_module_regex": "text_model + lm_expert q/k/v/o/gate/up/down",
    "linear_call_scales": text.get("linear_call_scales", []) + expert.get("linear_call_scales", []),
}
output_path.parent.mkdir(parents=True, exist_ok=True)
output_path.write_text(json.dumps(merged, indent=2), encoding="utf-8")
print(json.dumps({k: v for k, v in merged.items() if k != "linear_call_scales"}, indent=2))
print(f"linear_call_scales: {len(merged['linear_call_scales'])}")
PY
else
  [[ -f "$SCALES_JSON" ]] || die "missing SmoothQuant alpha=0.85 lm_expert scales: $SCALES_JSON"
fi
...
"$PYTHON" - "$QDQ_ONNX" "$EXPECTED_W8A8_NODES" <<'PY'
import json
import sys
from pathlib import Path
...
if actual != expected or smooth != expected or casts != expected:
    raise SystemExit(
        f"unexpected SmoothQuant full A8W8 Q/DQ coverage in {report_path}: "
        f"rewritten_linear_nodes={actual}, expected={expected}; "
        f"smoothquant_rewritten_linear_nodes={smooth}, expected={expected}; "
        f"output_cast_nodes={casts}, expected={expected}"
    )
PY
...
if [[ "$RUN_PROFILE" == "true" ]]; then
  ...
  "$PYTHON" "$SCRIPT_DIR/trt_sample_actions_core_deploy.py" profile \
    ...
    --iters "$PROFILE_ITERS" \
    --task "$TASK_SUITE SmoothQuant alpha=0.85 full A8W8 action-only profile" \
    "${CUDA_GRAPH_ARGS[@]}"
fi
...
if [[ "$RUN_NUMERIC_COMPARE" == "true" ]]; then
  "$PYTHON" "$SCRIPT_DIR/diagnose_3_4_numeric_baseline.py" compare \
    ...
    --output-dir "$OUTPUT_ROOT/numeric_compare_abc"
fi
...
rows = [
    ...
    {
        "config": "SmoothQuant alpha=0.85 full A8W8 action-only TensorRT",
...
printf '%s\n' "step 5O SmoothQuant alpha=0.85 full A8W8 CUDA Graph deployment outputs:" \
  ...
  "  channel calibration: $CHANNEL_SCALES_JSON" \
  ...
  "  A/B/C numeric compare: $OUTPUT_ROOT/numeric_compare_abc" \
