...
OUTPUT_ROOT="$REPO_ROOT/runs/deploy/3-5/B-precision-obey-bf16"
...
usage() {
  printf '%s\n' "Usage: $0 [options]" \
    ...
    "  --engine-output PATH       default: OUTPUT_ROOT/smolvla_debug_core_native_mixed_precision_obey.plan" \
...
mkdir -p "$OUTPUT_ROOT"
ENGINE_OUTPUT="${ENGINE_OUTPUT:-$OUTPUT_ROOT/smolvla_debug_core_native_mixed_precision_obey.plan}"
...
if [[ "$BUILD_ENGINE" == "true" ]]; then
  ...
  trtexec \
    ...
    --timingCacheFile="$OUTPUT_ROOT/smolvla_debug_bf16_precision_obey.cache"
...
printf '%s\n' "step 3-5B precision-obey BF16 outputs:" \
  "  BF16 source ONNX: $BF16_ONNX" \
  "  ORT-compatible BF16 shadow ONNX: $ORT_BF16_ONNX" \
  "  TensorRT precision-obey engine: $ENGINE_OUTPUT" \
