...
"""Summarize Step 5H fake-dequant calibration sample-size sweep."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sample-sizes", nargs="+", required=True)
...
def main() -> None:
    ...
    for sample_size in args.sample_sizes:
        run_dir = output_root / f"samples_{sample_size}"
        numeric = load_json(run_dir / "fake_dequant_numeric" / "numeric_baseline_summary.json")
        calibration = load_json(run_dir / f"activation_scales_{sample_size}.json")
        row: dict[str, Any] = {
        ...
        if numeric is not None:
            ...
            add_output_metrics(row, "suffix_out_step_09", find_metric(critical, "suffix_out_step_09"))
    ...
    rows = sorted(rows, key=lambda item: int(item["samples"]))
    summary = {
        "output_root": str(output_root),
        "sample_sizes": [int(item) for item in args.sample_sizes],
        "rows": rows,
    }
    output_root.mkdir(parents=True, exist_ok=True)
    with open(output_root / "calibration_sample_size_sweep_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    with open(output_root / "calibration_sample_size_sweep_summary.csv", "w", newline="") as f:
