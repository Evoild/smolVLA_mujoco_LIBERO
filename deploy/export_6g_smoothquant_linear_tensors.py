#!/usr/bin/env python3

"""Export a real SmoothQuant W8A8 Linear case for Step 6G CUDA kernels."""

from __future__ import annotations

import argparse
import json
from pathlib import Path

import torch
import torch.nn.functional as F
from torch import nn

from export_6f_layer0_attention_tensors import (
    collect_one_input,
    device_inputs,
    input_dtype,
    load_policy,
)
from linear_only_quant import load_activation_scales_by_module, quantize_weight_per_out_channel
from profile_6b_attention_breakdown import SmolVLADebugCoreWrapper
from lerobot.utils.random_utils import set_seed


TARGET_MODULE = "model.vlm_with_expert.vlm.model.text_model.layers.0.self_attn.q_proj"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--policy-path", default="smolvla_libero")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--seed", type=int, default=1000)
    parser.add_argument("--task", default="libero_spatial")
    parser.add_argument("--input-source", choices=["rollout", "synthetic"], default="rollout")
    parser.add_argument("--token-length", type=int, default=48)
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--sample-stride", type=int, default=1)
    parser.add_argument("--max-parallel-tasks", type=int, default=10)
    parser.add_argument("--model-dtype", choices=["bf16", "fp32"], default="bf16")
    parser.add_argument("--input-dtype", choices=["bf16", "fp32"], default="bf16")
    parser.add_argument(
        "--activation-scales-json",
        default="runs/deploy/5/O-smoothquant-alpha085-full-w8a8-deploy-cudagraph/smoothquant_alpha_0.85_text_and_lm_expert_activation_scales.json",
    )
    parser.add_argument("--target-module", default=TARGET_MODULE)
    parser.add_argument("--warmup", type=int, default=20)
    parser.add_argument("--iters", type=int, default=200)
    return parser.parse_args()


def write_binary(path: Path, tensor: torch.Tensor) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    array = tensor.detach().contiguous().cpu().numpy()
    path.write_bytes(array.tobytes())


def tensor_metrics(ref: torch.Tensor, got: torch.Tensor) -> dict[str, float]:
    ref_f = ref.detach().to(torch.float64).flatten()
    got_f = got.detach().to(torch.float64).flatten()
    diff = got_f - ref_f
    denom = torch.linalg.vector_norm(ref_f).clamp_min(1.0e-12)
    cosine = F.cosine_similarity(ref_f, got_f, dim=0)
    return {
        "cosine": float(cosine.item()),
        "relative_l2": float((torch.linalg.vector_norm(diff) / denom).item()),
        "max_abs": float(diff.abs().max().item()),
    }


def time_cuda(args: argparse.Namespace, fn) -> float:
    for _ in range(args.warmup):
        _ = fn()
    torch.cuda.synchronize()
    start = torch.cuda.Event(enable_timing=True)
    end = torch.cuda.Event(enable_timing=True)
    start.record()
    for _ in range(args.iters):
        _ = fn()
    end.record()
    torch.cuda.synchronize()
    return float(start.elapsed_time(end) / max(1, args.iters))


def main() -> None:
    args = parse_args()
    set_seed(args.seed)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    policy, preprocessor, device = load_policy(args.policy_path, args.device, args.model_dtype)
    scales_by_module = load_activation_scales_by_module(args.activation_scales_json, (rf"^{args.target_module}$",))
    scale_entry = scales_by_module[args.target_module]
    if not isinstance(scale_entry, dict) or "smooth_scale" not in scale_entry:
        raise ValueError(f"target module does not have SmoothQuant scale: {args.target_module}")
    activation_scale = float(scale_entry["scale"])
    smooth_scale = torch.as_tensor(scale_entry["smooth_scale"], device=device, dtype=torch.float32).clamp_min(1.0e-8)

    record = collect_one_input(args, policy, preprocessor, device)
    inputs = device_inputs(record["core_inputs"], device)
    wrapper = SmolVLADebugCoreWrapper(policy).to(device=device).eval()
    model = wrapper.model
    prefix_embs, _, _ = wrapper.embed_prefix_from_image_embeds(*inputs[:-1])
    text_model = model.vlm_with_expert.get_vlm_model().text_model
    layer0 = model.vlm_with_expert.get_model_layers([text_model, model.vlm_with_expert.lm_expert])[0][0]
    linear = layer0.self_attn.q_proj
    if not isinstance(linear, nn.Linear):
        raise TypeError(f"expected nn.Linear, got {type(linear)}")

    with torch.no_grad():
        norm_states = layer0.input_layernorm(prefix_embs)
        x = norm_states.reshape(-1, norm_states.shape[-1]).to(torch.float32)
        weight = linear.weight.detach().to(device=device, dtype=torch.float32)
        bias = linear.bias.detach().to(device=device, dtype=torch.float32) if linear.bias is not None else None
        smoothed_weight = weight * smooth_scale[None, :]
        qweight, weight_scale = quantize_weight_per_out_channel(smoothed_weight.cpu())
        qweight = qweight.to(device=device)
        weight_scale = weight_scale.to(device=device, dtype=torch.float32)

        x_smooth = x / smooth_scale
        qx = torch.round(x_smooth / activation_scale).clamp(-128, 127).to(torch.int8)
        x_deq = qx.to(torch.float32) * activation_scale
        w_deq = qweight.to(torch.float32) * weight_scale[:, None]
        ref = F.linear(x_deq, w_deq, bias)
        ref_2 = F.linear(x_deq, w_deq, bias)
        latency_ms = time_cuda(args, lambda: F.linear(x_deq, w_deq, bias)) if device.startswith("cuda") else 0.0

    files = {
        "x": "x.fp32.bin",
        "qweight": "qweight.int8.bin",
        "weight_scale": "weight_scale.fp32.bin",
        "bias": "bias.fp32.bin",
        "smooth_scale": "smooth_scale.fp32.bin",
        "reference": "reference.fp32.bin",
    }
    write_binary(output_dir / files["x"], x.to(torch.float32))
    write_binary(output_dir / files["qweight"], qweight.to(torch.int8))
    write_binary(output_dir / files["weight_scale"], weight_scale.to(torch.float32))
    write_binary(output_dir / files["bias"], bias.to(torch.float32) if bias is not None else torch.zeros(weight.shape[0], device=device))
    write_binary(output_dir / files["smooth_scale"], smooth_scale.to(torch.float32))
    write_binary(output_dir / files["reference"], ref.to(torch.float32))

    meta = {
        "policy_path": args.policy_path,
        "task": args.task,
        "seed": args.seed,
        "input_source": args.input_source,
        "sample_index": int(record.get("sample_index", 0)),
        "target_module": args.target_module,
        "activation_scales_json": args.activation_scales_json,
        "smoothquant_alpha": scale_entry.get("smoothquant_alpha"),
        "activation_scale": activation_scale,
        "quantization": scale_entry.get("quantization"),
        "shape": {
            "m": int(x.shape[0]),
            "k": int(x.shape[1]),
            "n": int(weight.shape[0]),
        },
        "reference_latency_ms": latency_ms,
        "reference_self_metrics": tensor_metrics(ref, ref_2),
        "files": files,
    }
    (output_dir / "smoothquant_linear_tensors_meta.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    print(json.dumps(meta, indent=2))


if __name__ == "__main__":
    main()
