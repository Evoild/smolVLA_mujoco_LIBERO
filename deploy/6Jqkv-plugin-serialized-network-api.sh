...
OUTPUT_ROOT="runs/deploy/6/J-qkv-plugin-serialized-network-api"
...
cmake --build "${BUILD_DIR}" --target smolvla_smoothquant_qkv_plugin_serialized_smoke -j "${JOBS:-8}"
...
"${BUILD_DIR}/smolvla_smoothquant_qkv_plugin_serialized_smoke" "${TENSOR_ROOT}" "${OUTPUT_ROOT}" "${Q_SCALE}" "${K_SCALE}" "${V_SCALE}" "${WARMUP}" "${ITERS}" \
  2>&1 | tee "${OUTPUT_ROOT}/smoothquant_qkv_plugin_serialized_smoke.log"
...
report_path = root / "smoothquant_qkv_plugin_serialized_smoke_report.json"
if meta_path.exists():
