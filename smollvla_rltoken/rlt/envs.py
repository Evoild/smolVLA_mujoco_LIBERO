...
class MockManipEnv:
    ...
    def __init__(
        ...
        self.state_dim = state_dim or action_dim
    ...
    def reset(self) -> dict:
        self._state = torch.rand(self.state_dim, generator=self.rng) * 1.6 - 0.8
        self._t = 0
        return self._obs()

    def step(self, action: Tensor) -> tuple[dict, float, bool]:
        action = action.detach().cpu().clamp(-1, 1)
        delta = torch.zeros_like(self._state)
        delta[: self.action_dim] = action[: self.action_dim]
        self._state = (self._state + 0.05 * delta).clamp(-1, 1)
        self._t += 1
        dist = self._state[:2].norm().item()
        success = dist < self.success_eps
        reward = 1.0 if success else 0.0
        # `done` signals task success only; time-limit truncation is handled
        # by the rollout worker (it must not zero the TD bootstrap).
